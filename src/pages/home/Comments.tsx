...
const comments: Comment[] = [
  {
    id: 1,
    name: "jay__1304",
    comment: "I was there, it was such a vibe. Props to stepfun and camilla 🙌",
    rating: 5,
  },
  {
    id: 2,
    name: "itsankitadey",
    comment: "Come to Guwahati please ❤🔥",
    rating: 4,
  },
  {
    id: 3,
    name: "avenezuelandreamer",
    comment: "All what I need in life 😍",
    rating: 4,
  },
    ...
    <div className="flex">
      {[...Array(5)].map((_, index) => (
        <Star
          key={index}
          className={`h-4 w-4 ${
            index < rating ? "text-yellow-600/80" : "text-gray-500"
          }`}
          ...
          <div className="grid grid-cols-2 gap-4 overflow-hidden h-[400px] mt-5">
            ...
            >
              ...
              {duplicatedComments.map((comment, index) => (
                ...
                >
                  <h3 className="text-sm md:text-lg font-semibold md:font-bold truncate text-left">
                    {comment.name}
                  </h3>
                  <StarRating rating={comment.rating} />
                  <p className="text-sm mt-2 text-left">{comment.comment}</p>
                </motion.div>
              ))}
            </motion.div>
            <motion.div
