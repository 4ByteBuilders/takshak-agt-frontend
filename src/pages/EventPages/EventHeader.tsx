...
export default function EventHeader({ event, scrollY }: EventHeaderProps) {
    ...
    return (
        ...
        >
            ...
            />
            <div className="absolute inset-0 flex flex-row text-start" style={backgroundStyle}>
                <div className="flex flex-col justify-end text-start p-10">
                    <h1 className="text-7xl font-bold transition-transform duration-300">{event.title}</h1>
                ...
                <div className={scrollY > 0 ? "hidden p-10" : "flex items-end p-10"}>
                    <Lottie animationData={scrolldown} style={{ width: 50, height: 50 }} />
                </div>
            </div>
        </motion.div>
    );
